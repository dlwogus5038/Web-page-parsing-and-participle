...
int My_BatchSearch::startSearch(My_AVL* &MAVL, int maxDicNum)
{
	ifstream inputQuery("query.txt");
	ofstream outputQuery("result.txt");
	string inputString;
	string readQuery;
	int inputSize = 0;
	int i = 0;
	...
	{
		getline(inputQuery, inputString); //读入一行
